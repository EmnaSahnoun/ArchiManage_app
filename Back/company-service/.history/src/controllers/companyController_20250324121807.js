...
const createCompany = async (req, res) => {
  try {
    ...
    const groupRes = await axios.post(
      `${KEYCLOAK_ADMIN_CONFIG.baseUrl}/admin/realms/${KEYCLOAK_ADMIN_CONFIG.realm}/groups`,
      { name },
      { headers: KEYCLOAK_ADMIN_CONFIG.headers }
    );
    ...
    const newCompany = new Company({
      ...
      email,
      phone,
      keycloakGroupId: groupRes.data.id
    });
    
    await newCompany.save();

    // 3. Ajouter l'utilisateur créateur au groupe
    await axios.put(
      `${KEYCLOAK_ADMIN_CONFIG.baseUrl}/admin/realms/${KEYCLOAK_ADMIN_CONFIG.realm}/users/${req.user.sub}/groups/${groupRes.data.id}`,
      {},
      { headers: KEYCLOAK_ADMIN_CONFIG.headers }
    );
    ...
    console.error(error);
    res.status(500).json({ message: error.response?.data?.errorMessage || error.message });
  }
};
